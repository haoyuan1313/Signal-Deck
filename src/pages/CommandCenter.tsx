...
function resolvePrice(prices: Record<string, number>, symbol: string): number | null {
  ...
  return (
    <div className="space-y-6 md:space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <KPICard title="Avg R / Trade" value={`${avgR}R`} subtext={`Past 30d`} icon={Brain} colorClass="text-zinc-400" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-6 overflow-hidden flex flex-col md:max-h-[480px] backdrop-blur-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-[10px] text-zinc-500 font-mono">Last 10</span>
          </div>
          <div className="flex-1 space-y-3 overflow-y-auto pr-1 custom-scrollbar">
            {signals.length > 0 ? signals.slice(0, 10).map((signal, idx) => (
              <motion.div
                key={signal.id}
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {openTrades.length > 0 ? openTrades.map(trade => {
                  ...
                  const slInit = trade.sl_init && trade.sl_init !== 0
                    ...
                    ? trade.sl
                    : null;
                  const riskAmt = slInit !== null ? Math.abs(trade.entry - slInit) : null;
                  ...
                  return (
                    <tr key={trade.id} className="hover:bg-zinc-800/20 transition-all group cursor-pointer">
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-violet-950/30 border border-violet-500/20 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
      >
        <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="text-[10px] font-bold uppercase tracking-widest text-violet-400 hover:text-violet-300 px-3 py-2 rounded-lg border border-violet-500/30 hover:border-violet-500/50 transition-all bg-violet-500/10"
        >
          View Agent →
        </button>
      </motion.div>
